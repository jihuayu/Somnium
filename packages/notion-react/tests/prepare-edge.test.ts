import test from 'node:test'
import assert from 'node:assert/strict'
import { prepareNotionRenderModel } from '../src/index'
import type { NotionDocument } from '../src/types'

const edgeDocument: NotionDocument = {
  pageId: 'root-page',
  rootIds: ['code-mermaid', 'link-preview', 'paragraph-1', 'link-to-page', 'child-page'],
  blocksById: {
    'code-mermaid': {
      id: 'code-mermaid',
      type: 'code',
      code: {
        language: 'mermaid',
        rich_text: [{ type: 'text', plain_text: 'graph TD;A-->B;' }]
      }
    },
    'link-preview': {
      id: 'link-preview',
      type: 'link_preview',
      link_preview: { url: 'https://cards.example.com/preview' }
    },
    'paragraph-1': {
      id: 'paragraph-1',
      type: 'paragraph',
      paragraph: {
        rich_text: [{
          type: 'mention',
          plain_text: 'Mention',
          mention: {
            type: 'link_mention',
            link_mention: {
              href: 'https://www.notion.so/workspace/Page-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'
            }
          }
        }]
      }
    },
    'link-to-page': {
      id: 'link-to-page',
      type: 'link_to_page',
      link_to_page: {
        type: 'page_id',
        page_id: 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa'
      }
    },
    'child-page': {
      id: 'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb',
      type: 'child_page',
      child_page: { title: 'Child' }
    }
  },
  childrenById: {
    'root-page': ['code-mermaid', 'link-preview', 'paragraph-1', 'link-to-page', 'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb']
  },
  toc: []
}

test('prepareNotionRenderModel handles null and resolver failures', async () => {
  const nullResult = await prepareNotionRenderModel(null)
  assert.equal(nullResult, null)

  const model = await prepareNotionRenderModel(edgeDocument, {
    resolveLinkPreview: async () => {
      throw new Error('network')
    },
    resolvePageHref: async () => {
      throw new Error('resolver')
    },
    initialPagePreviewMap: {
      stale: {
        url: '/stale',
        hostname: 'example.com',
        title: 'Stale',
        description: '',
        image: '',
        icon: ''
      }
    }
  })

  assert.ok(model)
  assert.equal(model?.highlightedCodeByBlockId['code-mermaid'], undefined)
  assert.equal(model?.linkPreviewMap['https://cards.example.com/preview'], undefined)
  assert.equal(model?.pageHrefMap['aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'], 'https://www.notion.so/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa')
  assert.equal(model?.pageHrefMap['bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb'], 'https://www.notion.so/bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb')
  assert.equal(model?.pagePreviewMap.stale, undefined)
})
