...
function getBlockClassName(blockId) {
...
/**
 * Official Notion API document renderer.
 *
 * @param {{
 *   document: {
 *     rootIds: string[],
 ...
 * }} props
 */
export default function NotionRenderer ({ document }) {
  ...
  const renderBlock = block => {
    ...
    switch (block.type) {
      ...
      case 'image': {
        ...
        return (
          <figure key={block.id} className={cn(className, 'my-6')}>
            {/* Use native img because Notion signed URLs are short-lived and hostnames vary. */}
      ...
      case 'embed': {
        ...
        const url = embed.url
        const iframeUrl = getEmbedUrl(url)
  ...
  const renderBlockList = blockIds => {
    const nodes = []
